          ...
          <div className="w-6 h-4 relative flex justify-center items-center z-30 pointer-events-none">
            <span
              className={`absolute bg-white rounded-full transition-all duration-300 ease-out ${
                isOpen
                  ? "w-2 h-2 translate-y-0"
                  : "w-full h-[1.5px] -translate-y-[3px]"
              }`}
            />
        ...
        <div ref={menuListRef} className="h-0 opacity-0 hidden pb-3">
          <div className="relative flex flex-col mt-0">
            <div
              ...
              style={{
                top: "-12px",
                height: `calc(${currentDotY}px - 0px)`,
              }}
            />
            ...
            {navLinks.map((link, idx) => {
              ...
              return (
                <Link
                  ...
                  className="nav-item-content group flex items-center px-5 h-8 transition-colors relative z-10 no-underline"
                >
                  <div className="w-6" />
                  <span
                    className={`ml-3.5 text-[14px] font-normal tracking-wide transition-colors duration-300 ${
                      isTargeted
                        ? "text-white"
